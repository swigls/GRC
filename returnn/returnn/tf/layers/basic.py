...
def get_concat_sources_data_template(src_layers, out_dim=None, allow_broadcast_all_sources=NotSpecified, name=None):
    ...
    return common_source.copy_template_replace_dim_tag(
        name=name, axis=common_source.feature_dim_axis, new_dim_tag=out_dim
    )


def concat_sources_with_opt_dropout(
    src_layers,
    out_dim=None,
    dropout=0,
    dropout_axis=None,
    dropout_noise_shape=None,
    dropout_on_forward=False,
    ...
    if dropout_axis is not None:
        ...
        assert not dropout_noise_shape, "do not provide both dropout_axis %r and dropout_noise_shape %r" % (
            dropout_axis,
            dropout_noise_shape,
        )
...
class ConcatLayer(LayerBase):
    ...
    def __init__(self, sources, allow_broadcast=False, out_dim=None, **kwargs):
        ...
        def _copy_compatible(x, axis):
            ...
            dummy_ref = dummy_ref.copy_template_replace_dim_tag(axis=out_concat_axis, new_dim_tag=x.dim_tags[axis])
        ...
        self.output.placeholder = tf_util.concat_with_opt_broadcast(
            [src.placeholder for src in sources_data],
            axis=out_concat_axis,
...
class ScaledGradientLayer(CopyLayer):
    ...
    def __init__(self, scale, shift=None, scale_shift_by_sum_over_axis=None, clip_max_axis=None, **kwargs):
        ...
        if isinstance(scale, (int, float)) and scale == 0.0 and shift is None:
            self.output.placeholder = tf.stop_gradient(self.output.placeholder)
        else:
            scale_t = (
                scale.output.copy_compatible_to(self.output).placeholder if isinstance(scale, LayerBase) else scale
            )
            shift_t = (
                shift.output.copy_compatible_to(self.output).placeholder if isinstance(shift, LayerBase) else shift
            )
...
class SelectSearchSourcesLayer(InternalLayer):
    ...
    def __init__(self, search_choices_layer, sources, **kwargs):
        ...
        if not search_choices:
            ...
            assert not self.output.beam, (
                "no src %r search choices but beam?" % src,
                src.network.debug_search_choices(src),
            )
            ...
            def transform(v):
                ...
                for i, base_src_choices in enumerate(reversed(search_choices_seq)):
                    ...
                    v = select_src_beams(
                        v,
                        src_beams=base_src_choices.src_beams,
                        ...
                        % (
                            ...
                            len(search_choices_seq),
                            get_valid_scope_name_from_str(search_choices.owner.name),
                        ),
                    )
                    if tag:
                        tag.set_tag_on_size_tensor(
                            v, batch=self.output.batch.copy_set_beam(base_src_choices.get_beam_info())
                        )
    ...
    def __init__(self, activation, opts=None, **kwargs):
        ...
        if activation:
            if "softmax" in activation:
                assert (
                    not opts
                )  # do not set axis or anything. this handled automatically. we moved feature to last axis.
...
class BatchNormLayer(CopyLayer):
    ...
    def __init__(
        self,
        in_dim=None,
        use_shift=NotSpecified,
        use_std=NotSpecified,
        use_sample=NotSpecified,
        force_sample=NotSpecified,
        momentum=NotSpecified,
        epsilon=NotSpecified,
        update_sample_only_in_training=NotSpecified,
        delay_sample_update=NotSpecified,
        param_version=NotSpecified,
        gamma_init=NotSpecified,
        beta_init=NotSpecified,
...
class NormLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if scale:
            with self.var_creation_scope():
                scale_param = self.add_param(
                    tf_compat.v1.get_variable("scale", param_shape, initializer=tf.ones_initializer())
                )
            norm_x *= (
                Data(name="scale_param", dim_tags=param_dim_tags, placeholder=scale_param)
                .copy_compatible_to(self.output)
                .placeholder
            )
        if bias:
            with self.var_creation_scope():
                bias_param = self.add_param(
                    tf_compat.v1.get_variable("bias", param_shape, initializer=tf.zeros_initializer())
                )
            norm_x += (
...
class MathNormLayer(_ConcatInputLayer):
    ...
    def __init__(self, p, axis=NotSpecified, axes=NotSpecified, keep_dims=False, **kwargs):
        ...
        self.output.placeholder = ReduceLayer.reduce(x, mode="sum", axes=axes, keep_dims=keep_dims) ** (1.0 / p)
...
class SliceLayer(_ConcatInputLayer):
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, axis, sources=(), slice_start=None, slice_end=None, slice_step=None, out_dim=None, **kwargs
    ):
        """
        :param str name:
        :param Dim|str axis:
        ...
        if slice_end is not None:
            if slice_end >= 0:
                out_dim_ = Dim(
                    kind=dim_tag.kind,
                    description="%s:slice-end" % name,
                    dimension=slice_end - (slice_start or 0),
                    auto_generated=True,
                )
            else:  # slice_end < 0
...
class SliceNdLayer(_ConcatInputLayer):
    ...
    def __init__(self, size, start=None, min_size=None, axis="T", out_spatial_dim=None, **kwargs):
        ...
        if isinstance(start, int):
            ...
            gather_positions_data = gather_positions_data.copy_add_dim_by_tag(
                slice_tag, unbroadcast=True, axis=start_data.batch_ndim
            )
            # [start+0, start+1, ...]
            ...
            pad_mask_data = gather_positions_data.copy_template(name="%s_gather_positions" % self.name, dtype="bool")
            pad_mask_data.placeholder = pad_mask
            gather_positions_data.placeholder = gather_positions
            position = InternalLayer(
                network=self.network, name="%s_internal" % gather_positions_data.name, output=gather_positions_data
            )
    ...
    def get_out_data_from_opts(cls, name, sources=(), start=None, size=None, axis="T", out_spatial_dim=None, **kwargs):
        ...
        if isinstance(size, LayerBase):
            out_spatial_dim_ = None
            if size.output.placeholder is not None:
                out_spatial_dim_ = Dim.get_tag_from_size_tensor(size.output.placeholder)
            if not out_spatial_dim_:
                out_spatial_dim_ = Dim(
                    kind=Dim.Types.Spatial, description="sliced-time:%s" % name, dimension=None, auto_generated=True
                )
            out_spatial_dim_.set_dyn_size_ext_for_batch_ctx(
                batch=size.output.batch, ctx=size.output.control_flow_ctx, dyn_size_ext=size.output
            )
            if out_spatial_dim:
            ...
            out_spatial_dim_ = Dim(
                kind=Dim.Types.Spatial, description="sliced-time:%s" % name, dimension=size, auto_generated=True
            )
        ...
        position = InternalLayer(
            network=sources[0].network, name="%s_internal" % gather_positions_data.name, output=gather_positions_data
        )
        return GatherLayer.get_out_data_from_opts(
            name="%s_gather" % name, sources=sources, position=position, axis=axis
        )
...
class GatherLayer(_ConcatInputLayer):
    ...
    def __init__(self, position: Union[LayerBase, int], axis: Union[Dim, str], clip_to_valid: bool = False, **kwargs):
        ...
        common_axes_input, common_axes_position, input_axes, position_axes = self._get_common_input_position_axes(
            input_data, position_data, old_gather_axis
        )
        ...
        params = tf.transpose(
            ...
            common_axes_input
            + input_axes[: gather_axis - batch_dims]
            + [old_gather_axis]
            + input_axes[gather_axis - batch_dims :],
        )
    ...
    def _get_common_input_position_axes(cls, input_data, position_data, old_gather_axis):
        ...
        common_axes_input = [
            i
            for i, tag in enumerate(input_data.dim_tags)
            if tag in input_tags and tag in pos_tags and i != old_gather_axis
        ]
        ...
        assert set(common_axes_input) | set(specific_input_axes) | {old_gather_axis} == set(
            range(input_data.batch_ndim)
        )
    ...
    @classmethod
    def _translate_input_axis(cls, input_axis, old_gather_axis, common_axes_input, input_axes, position_axes):
        """
    ...
    @classmethod
    def _translate_position_axis(cls, position_axis, old_gather_axis, common_axes_position, position_axes):
        """
    ...
    def get_out_data_from_opts(cls, name, sources, position, axis, **kwargs):
        ...
        # determine all common axes of input_data and position_data
        common_axes_input, common_axes_position, input_axes, position_axes = cls._get_common_input_position_axes(
            input_data, position_data, old_gather_axis
        )
        batch_dims = len(common_axes_input)
        ...
        if (
            input_data.feature_dim_axis_or_unspecified is NotSpecified
            and position_data.feature_dim_axis_or_unspecified is NotSpecified
        ):
...
class ScatterNdLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        position,
        position_axis,
        output_dim_via_time_from=None,
        out_spatial_dim=None,
        filter_invalid_indices=False,
        **kwargs,
    ):
        """
        ...
        assert out_spatial_dim.is_dim_known(), "%s: out_spatial_dim %s must have a known (dynamic or static) dim" % (
            self,
            out_spatial_dim,
        )
        self.position = position
        ...
        self.output.placeholder = tf.scatter_nd(indices=nd_idxs, updates=input_v, shape=output_shape)
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, position, position_axis, output_dim_via_time_from=None, out_spatial_dim=None, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
...
class LinearLayer(_ConcatInputLayer):
        ...
        **kwargs,
        ...
        with self.var_creation_scope():
            ...
            if self.with_bias:
                ...
                b = self.add_param(
                    tf_compat.v1.get_variable(name="b", shape=(n_out,), dtype=tf.float32, initializer=bias_initializer)
                )
            else:
        ...
        with tf.name_scope("linear"):
            ...
            if self.input_data.sparse:
                ...
                self.input_data.is_batch_feature_major and is_gpu_available_in_session()
            ):  # CuDNN has fast version for this
        ...
        if self.activation:
            ...
            if (
                act_func in {tf.nn.softmax, tf.nn.log_softmax}
                and self.output.feature_dim_axis != self.output.batch_ndim - 1
            ):
    ...
    def _get_in_split_info(self):
        ...
        if not all(in_split_info) or sum(in_split_info) != n_in:
            print(
                "%s: Warning: input split dims %r unclear for sources %r?" % (self, in_split_info, self.sources),
                file=log.v3,
            )
            return None
...
class LengthLayer(LayerBase):
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, axis="T", add_time_axis=False, dtype="int32", sparse=False, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
...
class SoftmaxOverSpatialLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_before_activation = OutputWithActivation(
            energy, act_func=tf.nn.log_softmax if log_space else tf.nn.softmax
        )  # (...,T)
        self.output.placeholder = self.output_before_activation.y
        if use_time_mask:
            set_padding_info(self.output.placeholder, dim=self.output.dim_tags[axis], pad_value=0.0)
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, axis=None, start=None, window_start=None, window_size=None, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
...
class SeqLenMaskLayer(_ConcatInputLayer):
    ...
    def __init__(
        self, mask_value, axis="T", seq_len_source=None, start=None, window_start=None, window_size=None, **kwargs
    ):
        """
    ...
    def build_mask(
        ...
        if start:
            ...
            start_data = start.copy_compatible_to(x, check_sparse=False, check_dtype=False)  # adds dummy time-dim
        ...
        if window_start is not None:
            ...
            window_start = window_start.copy_compatible_to(
                x, check_sparse=False, check_dtype=False
            )  # adds dummy time-dim
...
class RandomStateInitLayer(LayerBase):
    ...
    def get_out_data_from_opts(cls, name, algorithm=None, out_dim=None, **kwargs):
        ...
        if algorithm in cls._state_size_dim_tags_cache:
            ...
            algo_state_dim = Dim(
                kind=Dim.Types.Feature, description="random_%s_state" % algo_type.name, dimension=state_size
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if explicit_state is None:
            ...
            if auto_update_state is True:
                ...
                if not isinstance(state_, tf.Variable):
                    ...
                    raise Exception(
                        "%s: explicit_state %s is not a tf.Variable but %s" % (self, explicit_state, state_)
                    )
        ...
        def _safe_call(f):
            ...
            return tf_util.cond(
                tf.greater(tf.reduce_prod(shape_), 0), f, lambda: tf.zeros(shape=shape_, dtype=self.output.dtype)
            )
...
class RandIntLayer(LayerBase):
    ...
    def __init__(self, shape, maxval, minval=0, dtype="int32", sparse_dim=None, seed=None, **kwargs):
        ...
        shape_ = [
            d.get_for_batch_ctx(batch, self.network.control_flow_ctx).get_dim_value() for d in self.output.dim_tags
        ]
        self.minval = minval
        self.maxval = maxval
    ...
    @classmethod
    def get_out_data_from_opts(cls, name, network, shape, maxval, minval=0, dtype="int32", sparse_dim=None, **kwargs):
        """
        :param str name:
        :param returnn.tf.network.TFNetwork network:
        :param tuple[Dim|int]|list[Dim|int] shape: desired shape of output tensor
...
class RangeLayer(LayerBase):
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, limit, start=0, delta=1, dtype=None, sparse=False, out_spatial_dim=None, **kwargs
    ):
        """
        :param str name:
        :param int|float limit:
...
class RangeFromLengthLayer(LayerBase):
    ...
    def get_out_data_from_opts(cls, name, sources, dtype="int32", sparse=False, out_spatial_dim=None, **kwargs):
        ...
        source = sources[0].output
        assert source.dtype in {"int32", "int64"}, "%s %r: expect int32/int64 input but got %s from %s" % (
            cls.__name__,
            name,
            source.dtype,
            source,
...
class GatingLayer(_ConcatInputLayer):
    ...
    def get_out_data_from_opts(cls, name, sources, n_out=NotSpecified, out_dim=None, **kwargs):
        ...
        return Data(
            ...
            dim_tags=[out_dim_ if i == input_data.feature_dim_axis else d for (i, d) in enumerate(input_data.dim_tags)],
            sparse=False,
...
class WindowLayer(_ConcatInputLayer):
        ...
        **kwargs,
        ...
        if is_axis_from_description_recurrent(axis=axis, network=self.network, data=data):
            ...
            next_state = tf.concat(
                [prev_state[:, 1:], tf.expand_dims(data.placeholder, axis=1)], axis=1
            )  # (batch,window,...)
    ...
    @classmethod
    def get_rec_initial_extra_outputs(
        cls, network, batch_dim, rec_layer, window_size=None, window_dim=None, axis="T", sources=(), **kwargs
    ):
        """
        :param returnn.tf.network.TFNetwork network:
...
class MergeDimsLayer(_ConcatInputLayer):
    ...
    def __init__(self, axes, keep_order=NotSpecified, n_out=None, out_dim=None, **kwargs):
        ...
        if len(axes) > 1:
            ...
            x = tf.reshape(x, shape=shape[:i0] + [res_dim] + shape[i1:])
    ...
    def _get_merge_axes(cls, axes, keep_order, input_data, name):
        ...
        if keep_order:
            ...
            for axis in axes:
                ...
                assert len(axis_) <= 1, "%s: unique axes %r required in input %s, but got %r -> %r" % (
                    name,
                    axes,
                    input_data,
                    axis,
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, axes, keep_order=NotSpecified, sources=(), n_out=NotSpecified, out_type=None, out_dim=None, **kwargs
    ):
        """
        :param str name:
        :param typing.Sequence[Dim|str] axes:
...
class SplitLayer(_ConcatInputLayer):
    ...
    @classmethod
    def _get_axis_size_splits_num_splits(
        cls, name, input_data, axis=None, num_splits=None, size_splits=None, out_dims=None, err_prefix=None
    ):
        """
        :param str name:
        :param Data input_data:
        ...
        if not out_dims:
            ...
            out_dims = [
                Dim(
                    kind=input_data.dim_tags[axis].kind,
                    description="%s_split%i" % (name, idx),
    ...
    def get_sub_layer_out_data_from_opts(cls, layer_name, parent_layer_kwargs):
        ...
        out = cls._get_split_out_data(name=name, idx=idx, input_data=input_data, out_dims=out_dims, axis=axis)
...
class SplitDimsLayer(_ConcatInputLayer):
    ...
    def __init__(self, axis, dims, pad_to_multiples=None, pad_value=0, **kwargs):
        ...
        if pad_to_multiples and (not isinstance(rem_const_size, int) or rem_const_size != 1):
            ...
            if rem_dim.dimension is None and not rem_dim.is_batch_dim():
                ...
                rem_dim.dyn_size_ext = old_dim.dyn_size_ext.copy(
                    name="%s_dyn_size_ext" % (rem_dim.description or self.name)
                )
                rem_dim.dyn_size_ext.placeholder = -(
                    -data.get_dynamic_size(axis) // rem_const_size
                )  # == ceildiv(size, constant_size)
    ...
    def _map_old_axis_to_new_axis(
        cls, split_axis, dims, rem_dim_idx, old_axis, old_dim, kind, use_remaining=True, split_offset=None
    ):
        """
    ...
    def get_out_data_from_opts(cls, name, axis, dims, pad_to_multiples=None, sources=(), **kwargs):
        ...
        rem_dim_indices = [
            ...
            if (isinstance(d, int) and d < 0) or (isinstance(d, Dim) and d == axis_dim_tag)
        ]
            ...
            if len(rem_dim_indices) == 1:
                ...
                if n == 1:
                    ...
                    rem_dim = Dim(
                        ...
                        batch=axis_dim_tag.batch,
                        control_flow_ctx=axis_dim_tag.control_flow_ctx,
                    )
                    if rem_dim.dimension is None and axis_dim_tag.dyn_size_ext is not None:
                        rem_dim.dyn_size_ext = axis_dim_tag.dyn_size_ext.copy_template(
                            name="%s_split_dims%i" % (name, rem_dim_idx)
                        )
                if resolved_dims:
        ...
        if data.time_dim_axis is None:
            ...
            out.time_dim_axis = cls._map_old_axis_to_new_axis(
                split_axis=axis,
                dims=resolved_dims,
                rem_dim_idx=rem_dim_idx,
                old_axis=data.time_dim_axis,
                old_dim=data.dim_tags[data.time_dim_axis],
                kind=Dim.Types.Spatial,
                use_remaining=True,
                split_offset=0,
            )
        if data.feature_dim_axis is not None:
            expected_out_feature_dim_axis = cls._map_old_axis_to_new_axis(
                ...
                dims=resolved_dims,
                rem_dim_idx=rem_dim_idx,
                old_axis=data.feature_dim_axis,
                old_dim=data.dim_tags[data.feature_dim_axis],
...
class ReshapeLayer(LayerBase):
    ...
    def __init__(self, in_dims, out_dims, extra_deps=(), **kwargs):
        ...
        self.output.placeholder = tf.reshape(data.placeholder, [d.get_dim_value() for d in dims])
        ...
        if x.dims[axis].need_masking():
            if batch_major:
                self.output.placeholder = tf_util.flatten_with_seq_len_mask(
                    x.placeholder,
                    seq_lens=x.get_dynamic_size(axis),
                    batch_dim_axis=x.batch_dim_axis,
                    time_dim_axis=axis,
                )
            else:
...
class UnflattenNdLayer(_ConcatInputLayer):
    ...
    def __init__(self, sizes, num_axes, in_dim="T", out_dims=None, declare_same_sizes_as=None, **kwargs):
        ...
        self.output.placeholder = tf_util.unflatten_nd(
            input_data.placeholder, sizes_data.placeholder, num_axes=num_axes
        )
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, num_axes, in_dim="T", out_dims=None, declare_same_sizes_as=None, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
...
class ExpandDimsLayer(_ConcatInputLayer):
    ...
    def _get_axis(cls, data, axis):
        ...
        if axis in ["f", "feature"]:
            assert not data.sparse
            if data.feature_dim_axis is None:
                return data.batch_ndim
            return data.feature_dim_axis + 1
    ...
    def get_out_data_from_opts(cls, name, axis, dim=1, sources=(), **kwargs):
        ...
        if isinstance(dim, Dim):
            ...
            new_dim = Dim(kind=kind, description="%s_expand_dims" % name, auto_generated=True, dimension=dim)
        data = data.copy_template(name="%s_output" % name)
...
class RepeatLayer(_ConcatInputLayer):
    ...
    def __init__(self, repetitions, axis="T", out_dim=None, **kwargs):
        ...
        assert self.output.have_batch_axis() == (
            self.input_data.have_batch_axis() or repetitions_data.have_batch_axis()
        )
        ...
        def copy_placeholder_with_batch_axis(data, other_batch):
            ...
            :rtype: Data
            :return: data, with shape [B, T, ...], maybe with added batch dim.
            """
            if self.output.have_batch_axis():
                ...
                data = data.copy_add_batch_dim(batch_dim_axis=0, batch=batch_info)
                return data  # [B=1, T, ...]

        input_data = copy_placeholder_with_batch_axis(
            self.input_data, other_batch=repetitions_data.batch
        )  # [B, T, ... ]
        repetitions_data = copy_placeholder_with_batch_axis(
            repetitions_data, other_batch=self.input_data.batch
        )  # [B, T]
        ...
        if repetitions_data.has_dynamic_size(dyn_axis) and repetitions_data.batch_ndim >= 2:
            ...
            repetitions_data.placeholder = tf_util.where_bc(
                mask, repetitions_data.placeholder, 0, name="repetitions_masked_axis_%i" % dyn_axis
            )
        ...
        if tag.is_dynamic() and tag.dyn_size is None:  # dynamic? dyn sizes needed?
        ...
        **kwargs,
        ...
        if set_sparse_dim is not NotSpecified:
            ...
            if isinstance(set_sparse_dim, Dim):
                ...
                out.sparse_dim = Dim(
                    kind=Dim.Types.Feature,
                    dimension=set_sparse_dim,
                    description="%s:set-sparse-dim" % name,
                    auto_generated=True,
                )
...
class ConvLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(in_spatial_dims_) == len(filter_size), "%s: in_spatial_dims %s not matching to filter_size %s" % (
            self,
            in_spatial_dims_,
            filter_size,
        )
        ...
        if padding == "SAME_STATIC":
            x = self.get_input_placeholder_with_same_static_padding(
                input_data=input_data,
        ...
        if groups > 1 and groups == n_in and len(filter_size) <= 2:  # depthwise conv
            ...
            y = tf_compat.v1.nn.conv2d(
                x, data_format=data_format, filter=filters, padding=padding, strides=strides, dilations=dilation_rate
            )
        ...
        if with_bias:
            if bias:
                ...
                with self.var_creation_scope():
                    ...
                    b = self.add_param(
                        tf_compat.v1.get_variable(name="bias", shape=(n_out,), initializer=bias_initializer)
                    )
                if out_batch_feature_major:
                    y += (
                        Data(name="bias", placeholder=b, dim_tags=[out_dim]).copy_compatible_to(self.output).placeholder
                    )
                else:
                    y += b
        if activation:
    ...
    def transform_input(
        ...
        if in_spatial_dims:
            ...
            assert sorted(set(axes)) == sorted(axes), "in_spatial_dims %s must be unique but map to %s" % (
                in_spatial_dims,
                axes,
            )
            if sorted(axes) != axes:
        ...
        if use_time_mask:
            ...
            x = input_data.placeholder
            for dim in in_spatial_dims:
                if not dim.need_masking():
                    continue
    ...
    def get_input_placeholder_with_same_static_padding(
        ...
        for axis, dim in enumerate(input_data.dims):
            ...
            if axis == num_batch_dims and out_batch_feature_major:
                # input_data has dimensions [batch] + [channels] + [spatial] * len(filter_size)
                continue
            if axis >= num_batch_dims + len(filter_size) and not out_batch_feature_major:
                # input_data has dimensions [batch] + [spatial] * len(filter_size) + [channels]
                break
            filter_size_for_axis = filter_size[axis - num_batch_dims - out_batch_feature_major]
            stride_size_for_axis = strides[axis - num_batch_dims - out_batch_feature_major]
            if dim.is_dynamic():
                # use static same padding
            ...
            if dim.is_dynamic():
                ...
                # thus use this for left_padding, to avoid that we ignore any frames at the end.
                left_padding = (filter_size_for_axis - stride_size_for_axis) // 2
                assert left_padding >= 0  # Not implemented yet
            else:
                # left padding is either the same as right padding or one less
                left_padding = padding_size_for_axis // 2
            right_padding = padding_size_for_axis - left_padding
            paddings[axis] = [left_padding, right_padding]
    ...
    def get_out_data_from_opts(
        ...
        **kwargs,
        ...
        if padding == "SAME_STATIC":
            padding = "SAME"  # "same_static" padding leads to the same output dimensions as "same" padding
        ...
        if out_spatial_dims:
            ...
            for i in range(len(filter_size)):
                ...
                dim_tags.append(
                    Dim(
                        kind=Dim.Types.Spatial,
                        description="%s:conv:s%i" % (name, i),
                        dimension=new_dim,
                        derived_from_tag=old_tag,
                        undefined=not old_tag,
                        auto_generated=True,
                    )
                )
        ...
        if len(old_spatial_dim_tags) == len(filter_size):
            cls.set_output_dim_tags(
                out,
                num_batch_dims=num_batch_dims,
                in_spatial_dims=old_spatial_dim_tags,
...
class PoolLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if padding == "SAME_STATIC":
            x = ConvLayer.get_input_placeholder_with_same_static_padding(
                input_data=input_data,
                num_batch_dims=num_batch_dims,
...
class TransposedConvLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if in_spatial_dims:
            assert [input_data.dim_tags[a] for a in spatial_axes] == [
        ...
        if with_bias:
            if bias:
                ...
                with self.var_creation_scope():
                    ...
                    b = self.add_param(
                        tf_compat.v1.get_variable(name="bias", shape=(self.output.dim,), initializer=bias_initializer)
                    )
                y += b
        if activation:
            act_func = get_activation_function(activation)
    ...
    def deconv_output_length(
        input_length, filter_size, padding, output_padding=None, stride=0, dilation=1, out_dim=None
    ):
        """
...
class ReduceLayer(_ConcatInputLayer):
    ...
    def __init__(
        self, mode, axes=None, axis=None, keep_dims=False, enforce_batch_dim_axis=None, use_time_mask=None, **kwargs
    ):
        """
            ...
            if x.batch_dim_axis in axes and x.time_dim_axis in axes and len(axes) == 2:
                ...
                for axis in axes:
                    ...
                    if x.placeholder.dtype.is_floating or x.placeholder.dtype.is_integer:
                        ...
                        raise TypeError(
                            "reduce: unexpected input type %r from input %s" % (x.placeholder.dtype, input_data)
                        )
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, mode="", axes=None, axis=None, keep_dims=False, enforce_batch_dim_axis=None, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
...
class StackLayer(LayerBase):
    ...
    def __init__(self, axis=None, out_spatial_dim=None, **kwargs):
        ...
        sources_ = [src.output.copy_compatible_to(common_source, unbroadcast=True) for src in self.sources]
...
class WeightedSumLayer(_ConcatInputLayer):
    ...
    def __init__(self, axes, padding=None, size=None, keep_dims=None, **kwargs):
        ...
        if keep_dims:
            ...
            y = tf.reshape(
                y, shape=[x_shape[i] for i in range(len(other_axes))] + [y_shape[i + 1] for i in range(len(axes))]
            )  # [shape of other_axes] + [shape of axes]
    ...
    def get_out_data_from_opts(cls, name, sources, axes, padding=None, size=None, keep_dims=None, **kwargs):
        ...
        if keep_dims:
            ...
            for i, a in enumerate(axes):
                dim_tags[a] = Dim(
                    kind=dim_tags[a].kind,
                    description="%s:weighted-sum:%i" % (name, i),
                    dimension=res_dims[i],
                    auto_generated=True,
                )
...
class ElemwiseProdLayer(_ConcatInputLayer):
    ...
    def __init__(self, axes, size=None, **kwargs):
        ...
        with self.var_creation_scope():
            w = self.add_param(
                tf_compat.v1.get_variable(name="W", shape=size, initializer=tf.constant_initializer(1.0))
            )
        w_full_shape = [self.input_data.batch_shape[a] if (a in axes) else 1 for a in range(self.input_data.batch_ndim)]
...
class PrefixInTimeLayer(_ConcatInputLayer):
    ...
    def __init__(self, axis="T", out_dim=None, prefix=0.0, repeat=1, size_base=None, **kwargs):
        ...
        shape = [
            ((self.output.batch_shape[i] or tf.shape(input_data.placeholder)[i]) if (i != axis_int) else max_repeat)
    ...
    def get_out_data_from_opts(cls, name, sources, axis="T", out_dim=None, size_base=None, repeat=1, **kwargs):
        ...
        if size_base:
            ...
            out_dim_ = (
                repeat if isinstance(repeat, int) else SpatialDim("%s:repeat" % repeat.name, auto_generated=True)
            ) + in_dim
        if out_dim:
            out_dim_.declare_same_as(out_dim)
...
class PostfixInTimeLayer(_ConcatInputLayer):
    ...
    def __init__(self, axis="T", out_dim=None, postfix=0.0, repeat=1, **kwargs):
        ...
        if in_dim.dyn_size is not None:  # dynamic
            ...
            assert set(in_dim.dyn_size_ext.dim_tags).issubset(
                self.output.dim_tags
            )  # https://github.com/rwth-i6/returnn/issues/721
...
class TimeChunkingLayer(_ConcatInputLayer):
    ...
    def __init__(self, chunk_size, chunk_step, axis="T", out_dim=None, **kwargs):
        ...
        if in_dim.dyn_size is not None:
            ...
            index = tf.fill(tf.shape(x_t)[:2], 1.0)
        ext_rem_shape = None
        if x.batch_ndim != 3:
...
class DotLayer(LayerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if red1 == a_out.sparse_dim:
            ...
            assert a_reduce_axes and b_reduce_axes, "%s: sources %r, red1 %r, red2 %r" % (
                self,
                self.sources,
                red1,
                red2,
    ...
    def get_out_data_from_opts(
        ...
        **kwargs,
        ...
        if not b_var_dims and add_var2_if_empty:
            b_var_dims.append(SpatialDim("%s:dot:dummy-var2" % name, dimension=1, auto_generated=True))
        ...
        if adjust_size_info and axis_wob in self.output.size_placeholder:
            ...
            if amount < 0:
                ...
                if pad:
                    ...
                    size_delta = 0
            new_size = tf.clip_by_value(self.output.size_placeholder[axis_wob] + size_delta, 0, tf.shape(shifted)[axis])
            from ..util.data import Dim

            Dim(
                kind=Dim.Types.Spatial,
                description="%s_shift_axis" % self.name,
                dyn_size=new_size,
...
class ResizeLayer(_ConcatInputLayer):
    ...
    def __init__(self, factor, axis, out_dim=None, kind="nn", fill_value=None, fill_dropout=None, **kwargs):
        ...
        if isinstance(factor, (int, float)):
            ...
            assert factor.output.dtype in ("int32", "float32"), "%s: factor must be int or float, got %s" % (
                self,
                factor,
            )
        ...
        if fill_dropout:
            ...
            random_tensor += tf_compat.v1.random_uniform(
                [shape[axis], factor - 1], seed=self.network.random.randint(2**31)
            )  # (old_size, factor - 1)
            ...
            x = tf.reshape(
                x, [shape[0], new_size_dropped] + remaining_shape
            )  # [batch, new_size_dropped] + remaining_shape
            out_dyn_size = input_data.dim_tags[axis].dyn_size
            if out_dyn_size is not None:
                out_dyn_size = out_dyn_size * factor
                orig_mask = tf.sequence_mask(out_dyn_size, maxlen=new_size, dtype=tf.bool)  # (batch,new_size)
...
class RemoveLayer(LayerBase):
    ...
    def get_out_data_from_opts(cls, name, sources, axis="T", out_dim=None, **kwargs):
        ...
        if not out_dim:
            out_dim = Dim(
                kind=in_dim.kind,
                description="%s_removed_items",
                dimension=None,
                derived_from_tag=in_dim,
                auto_generated=True,
            )
...
class CombineLayer(LayerBase):
        ...
        **kwargs,
        ...
        if eval_for_output_loss:
            ...
            self.output_loss = self._op_kind_eval(
                sources=[
                    InternalLayer(name=layer.name, network=self.network, output=Data.from_tensor(layer.output_loss))
        ...
        if with_bias:
            with self.var_creation_scope():
                b = self.add_param(
                    tf_compat.v1.get_variable(
                        name="b", shape=(self.output.dim,), initializer=tf.constant_initializer(value=0)
                    )
                )
            x += b
        if activation:
    ...
    def _op_kind_eval(self, sources, eval_str, eval_locals=None):
        ...
        if callable(eval_str):
            ...
            x = eval(eval_str, vs)
        assert sorted(used_sources) == list(range(len(sources))), "not used sources: %r" % set(
            range(len(sources))
        ...
        **kwargs,
        ...
        if out_type:
            if isinstance(out_type, dict):
                ...
                raise TypeError("unexpected type of out_type %r" % (out_type,))
        return super(CompareLayer, cls).get_out_data_from_opts(
            n_out=n_out, out_type=out_type_, sources=sources, **kwargs
        )
    ...
    def __init__(self, condition, true_from, false_from, **kwargs):
        ...
        if isinstance(condition, bool):
            ...
            self.output.placeholder = where_bc(
                condition=condition.output.copy_compatible_to(
                    self.output, check_dtype=False, check_sparse=False
                ).placeholder,
                x=get_source_output(true_from, const_name="true_const_output")
                .copy_compatible_to(self.output)
                .placeholder,
                y=get_source_output(false_from, const_name="false_const_output")
                .copy_compatible_to(self.output)
                .placeholder,
            )
...
class CondLayer(LayerBase):
    ...
    def __init__(
        ...
        true_layer,
        false_layer,
        _condition_network=None,
        _true_layer_network=None,
        _false_layer_network=None,
        _extra_out=None,
        **kwargs,
    ):
        """
    ...
    def _cond_layer_return(self, layer, net_name):
        ...
        return (
            out.placeholder,
            extra_out,
            [out.size_placeholder[i] for i in sorted(out_template.size_placeholder.keys())],
        )
    ...
    def _transform_layer(cls, d, key, network, get_layer):
        ...
        extra_net = network.make_extra_net(
            prefix_name="extra._internal_template.%s(%s)" % (name, key),
            net_name="%s/%s(%s)" % (network.name, name, key),
            boundary=True,
            only_template=True,
        )
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        false_layer_class, false_layer_desc = cls._transform_layer(
            d, "false_layer", network=network, get_layer=get_layer
        )
...
class SearchSortedLayer(LayerBase):
    ...
    def __init__(self, sorted_sequence, values, axis="T", side="left", **kwargs):
        ...
        sorted_to_values_batch_axes = values_data.find_matching_dim_map(
            other=sorted_data, other_axes=sorted_batch_axes
        )  # sorted B -> values B
        values_batch_axes = [
            sorted_to_values_batch_axes[sorted_batch_ax] for sorted_batch_ax in sorted_batch_axes
        ]  # = values B
        ...
        self.output.placeholder = tf.searchsorted(
            sorted_sequence=x, values=transposed_values_data.placeholder, side=side, out_type=self.output.dtype
        )
    ...
    def get_out_data_from_opts(cls, sorted_sequence, values, axis, name, network, **kwargs):
        ...
        output_data = values_data.copy("%s_output" % name).copy_transpose(
            perm=values_batch_axes + values_non_batch_axes
        )
    ...
    def _update_for_rec_previous_layer(cls, rec_previous_layer, subnet_dict, subnet):
        ...
        for layer_name in list(subnet_dict.keys()):
            ...
            dummy_rec_previous_layer = InternalLayer(
                ...
                output=Data(
                    name="dummy_rec_previous_layer(%s)" % layer_name,
                    dim=1,
                    shape=(1,),
                    batch=rec_previous_layer.output.batch,
                    beam=rec_previous_layer.output.beam,
                ),
            )
    ...
    def get_rec_initial_extra_outputs(cls, batch_dim, rec_layer, encapsulate=False, **kwargs):
        ...
        for layer_name, sub_layer in subnet.layers.items():
            ...
            with cl.cls_setup_scope(**layer_desc):
                d = cl.get_rec_initial_extra_outputs(batch_dim=batch_dim, rec_layer=rec_layer, **layer_desc)
...
class LossLayer(LayerBase):
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        d["loss_"] = cls._make_loss(
            class_name=d.pop("loss_", None),
            opts=d.pop("loss_opts_", {}),
            network=network,
            get_layer=get_layer,
            always_make=True,
        )
...
class ForcedAlignmentLayer(_ConcatInputLayer):
    ...
    def __init__(self, align_target, topology, input_type, blank_idx=-1, blank_included=False, **kwargs):
        ...
        alignment, scores = fast_viterbi(
            am_scores=logits,
            am_seq_len=logits_data.get_sequence_lengths(),
            edges=edges,
            weights=weights,
...
class SparseSoftmaxCrossEntropyWithLogitsLayer(LayerBase):
    ...
    def __init__(self, logits, targets, axis=None, **kwargs):
        ...
        targets_data = targets.output.copy_compatible_to(
            self.output, check_sparse=False, check_dtype=False, add_dims=False
        )
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        """
        d.setdefault("from", [])
        super(SparseSoftmaxCrossEntropyWithLogitsLayer, cls).transform_config_dict(
            d, network=network, get_layer=get_layer
        )
...
class CtcLossLayer(LayerBase):
    ...
    def __init__(self, logits, targets, blank_index=-1, max_approx=False, **kwargs):
        ...
        self.targets = targets
        assert (
            targets.output.sparse_dim.dimension
            in
        ...
        **kwargs,
        ...
        if align_target == "ctc":
            ...
            fwdbwd, obs_scores = fast_baum_welch(
                am_scores=am_scores, float_idx=seq_mask, edges=edges, weights=weights, start_end_states=start_end_states
            )
            ...
            fwdbwd, obs_scores = fast_baum_welch_by_sprint_automata(
                sprint_opts=sprint_opts, tdp_scale=tdp_scale, am_scores=am_scores, float_idx=seq_mask, tags=seq_tags
            )
...
class FramewiseStatisticsLayer(LayerBase):
    ...
    def __init__(self, sil_label_idx, histogram_num_bins=20, **kwargs):
        ...
        for _k, _v in {
            ...
            "true_label_prob_histogram": true_label_prob_histogram,
        }.items():
            for _k2 in ["", "_sil", "_no_sil"]:
                ...
                with self.var_creation_scope():
                    acc_v = tf.Variable(
                        ...
                        initial_value=numpy.zeros(acc_shape, dtype=acc_dtype),
                        dtype=acc_dtype,
                        trainable=False,
                    )
...
class PrintLayer(LayerBase):
    ...
    def __init__(self, summarize=99, extra_print_args=(), **kwargs):
        ...
        with tf.name_scope("print_layer"):
            ...
            if (
                not tf_util.has_current_control_flow_context()
            ):  # Only possible to globally register if not in cond/loop.
...
class HDFDumpLayer(LayerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        extra = {
            key: output.copy_add_batch_dim(0, batch=data.batch) if not output.have_batch_axis() else output
        ...
        def py_write(data_np, tags, sizes, *extras):
            ...
            try:
                ...
                for i, (key, extra_data) in enumerate(sorted(self.extra.items())):
                    ...
                    value_seq_lens = {
                        i: size for (i, size) in zip(sorted(extra_data.size_placeholder.keys()), value_sizes)
                    }
                ...
                if self.use_fused and output_before_softmax_flat is not None:
                    ...
                    if self.label_smoothing:
                        out = smoothing_cross_entropy(
                            ...
                            gaussian=self.label_smoothing_gaussian,
                        )  # shape(labels)
                    else:
                    ...
                    print(
                        "Warning: using numerical unstable sparse Cross-Entropy loss calculation (%s to %s)"
...
class BinaryCrossEntropyLoss(Loss):
    ...
    def get_error(self):
        ...
        with tf.name_scope("loss_frame_error"):
            ...
            output_bool = tf.greater(self.output_flat, 0.0)  # logits
...
class CtcLoss(Loss):
    ...
    def __init__(
        self,
        target_collapse_repeated=False,
        auto_clip_target_len=False,
        output_in_log_space=False,
        beam_width=100,
        ctc_opts=None,
        use_native=False,
        use_viterbi=False,
    ...
    def _get_target_sparse_labels(self):
        ...
        labels = sparse_labels(
            self.target.placeholder, target_seq_lens, collapse_repeated=self.target_collapse_repeated
        )
        ...
        with tf.name_scope("loss_ctc"):
            ...
            if self.use_viterbi:
                ...
                self._ctc_loss = ctc_loss_viterbi(
                    ...
                    logits_time_major=self.output.is_time_major,
                    targets=self.target.get_placeholder_as_batch_major(),
                    targets_seq_lens=self.target_seq_lens,
                )
                ...
                self._ctc_loss = tf_compat.v1.nn.ctc_loss(
                    ...
                    time_major=self.output.is_time_major,
                    **self.ctc_opts,
                )
            ...
            if self.beam_width > 1:
                ...
                decoded = self.base_network.cond_on_train(
                    ...
                    lambda: tf.nn.ctc_beam_search_decoder(
                        inputs=logits, sequence_length=seq_lens, beam_width=self.beam_width
                    )[0][0],
                )
            else:
...
class EditDistanceLoss(Loss):
    ...
    def _debug_print_out(self):
        ...
        return [
            "output",
            tf.size(output),
            vocab_idx_repr(output, self.output),
            "target",
            tf.size(target),
            vocab_idx_repr(target, self.output),
        ]
...
class BleuLoss(Loss):
    ...
    def get_error(self):
        ...
        score = bleu_score(
            hypothesis=self.output.get_placeholder_as_batch_major(),
            hyp_seq_lens=self.output.get_sequence_lengths(),
            truth=self.target.get_placeholder_as_batch_major(),
            truth_seq_lens=self.target.get_sequence_lengths(),
        )
...
class ExpectedLoss(Loss):
    ...
    def __init__(
        ...
        loss_kind,
        norm_scores=True,
        norm_scores_stop_gradient=True,
        divide_beam_size=True,
        subtract_average_loss=True,
        ...
        with tf.name_scope("expected_loss"):
            ...
            if self.norm_scores:
                scores_norm_shift = tf.reduce_logsumexp(
                    beam_scores, name="scores_norm_shift", axis=1, keepdims=True
                )  # (batch,1)
            ...
            def compute_cost(s, start, c):
                ...
                v = tf.reshape(
                    tf.reshape(
                        chunk_out,
                        (
                            tf.shape(chunk_out)[0],
                            tf.shape(chunk_out)[1] // self._embedding_dimension,
                            self._embedding_dimension,
                        ),
                    ),
                    (
                        tf.shape(chunk_out)[0] * (tf.shape(chunk_out)[1] // self._embedding_dimension),
                        self._embedding_dimension,
                    ),
                )
                ...
                y = tf.reshape(
                    tf.reshape(
                        chunk_target,
                        (
                            tf.shape(chunk_target)[0],
                            tf.shape(chunk_target)[1] // self._nr_of_sources,
                            self._nr_of_sources,
                        ),
                    ),
                    (
                        tf.shape(chunk_target)[0] * (tf.shape(chunk_target)[1] // self._nr_of_sources),
                        self._nr_of_sources,
                    ),
                )
                chunk_c = (
...
class ExternSprintLoss(Loss):
    ...
    def get_value(self):
        ...
        with tf.name_scope("ExternSprintLoss"):
            ...
            if not self.output.is_time_major:
                output_before_softmax = swapaxes(
                    output_before_softmax, self.output.time_dim_axis, self.output.batch_dim_axis
                )
            ...
            loss = custom_gradient.generic_loss_and_error_signal(
                loss=loss, x=output_before_softmax, grad_x=error_signal
            )
            return loss
        ...
        with tf.name_scope("ViaLayerLoss"):
            ...
            if loss.shape.ndims == 0:  # scalar
                ...
                loss = tf.reshape(
                    loss,
                    [
                        orig_loss_shape[0] if i == self.output.batch_dim_axis else 1
                        for i in range(self.output.batch_ndim)
                    ],
                )
            else:
            ...
            loss = custom_gradient.generic_loss_and_error_signal(loss=loss, x=grad_wrt, grad_x=error_signal)
...
class SamplingBasedLoss(Loss):
    ...
    def _nce_loss(
        ...
        logits, labels = compute_sampled_logits(
            weights=weights,
            biases=biases,
            labels=labels,
            inputs=inputs,
        ...
        sampled_losses = tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=logits, name="sampled_losses")
...
def _init_layer_class_dict():
    ...
    for mod in [rec, variable, signal_processing, segmental_model, mine]:
