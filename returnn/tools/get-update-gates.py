...
def main(argv):
  ...
  argparser.add_argument("--layers", default=["energy"], action="append",
  ...
  print(network.get_extern_data(network.extern_data.default_input))
  print(network.get_extern_data(network.extern_data.default_target))
  for l in layers:
    ...
    if args.output_format == "npy":
      ...
      for i in range(n_batch):
        ...
        for l in [("rec_%s" % l) for l in layers]:
          assert l in kwargs
          out = kwargs[l][i]
          #out = np.reshape(out, [out.shape[0], out.shape[2]])
          energy = out #(U,T)
          exp_energy = np.exp(-energy)
          exp_energy_cumsum = np.cumsum(exp_energy, axis=1)
          update = 1. / (1.+exp_energy_cumsum)
          update[:,0] = 1.
          out = update
          assert out.ndim >= 2
